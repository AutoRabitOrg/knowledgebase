# IZ Scan Extension Configuration

### Generate License

1. Click on the "IZ" icon from the activity bar
2.  Click on **`IZ Scan Extension`** to request a trial license

    <figure><img src="../../../../../.gitbook/assets/new-connection (1).png" alt=""><figcaption></figcaption></figure>
3.  Enter the Email ID, Company Name, and click on **`Generate License`** <br>

    <figure><img src="../../../../../.gitbook/assets/generate-license.png" alt=""><figcaption></figcaption></figure>
4. A trial license will be generated for 15 days, and the license key will be sent to the email ID

### Apply License

1. Enter the license key shared in the email.
2.  Contact support@integralzone.com to get the developer token required to start the trial.

    <figure><img src="../../../../../.gitbook/assets/apply_license_v2.png" alt=""><figcaption></figcaption></figure>
3. Click on **`Apply License`**
4.  Once the license is applied, the Quality Profiles and corresponding rules will be loaded.

    <figure><img src="../../../../../.gitbook/assets/standalone_profiles.png" alt=""><figcaption></figcaption></figure>
5. [IZ Scan - Fly Results](fly-results.md) provides real-time visibility into project issues.

### See Also

* [Install IZ Scan for Cloud](install-vs-code-extension-cloud.md)
* [Install IZ Scan for Desktop](install-vs-code-extension-desktop.md)
