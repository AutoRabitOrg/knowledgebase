...
## What is a Unique Identifier (UID) in AutoRABIT Vault? <a href="#what-is-a-unique-identifier-uid-in-vault" id="what-is-a-unique-identifier-uid-in-vault"></a>
...
1. In **AutoRABIT Vault**, navigate to **Setup**, then select your **Salesforce Org**.
...
2. AutoRABIT Vault will scan and auto-select non-auto-increment unique fields across all objects.
3. This may take some time; AutoRABIT Vault notifies the user upon completion.
...
2. A dropdown will display all Salesforce Orgs registered in AutoRABIT Vault that have at least one UID mapping configured.
3. Select the destination Org and click **Clone**.
4. AutoRABIT Vault will check for the presence of identical objects and fields in the target Org and replicate the UID mappings.
