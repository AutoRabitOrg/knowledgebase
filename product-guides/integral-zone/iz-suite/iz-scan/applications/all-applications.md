...
* Available from IZ Suite Server **26.3.1** under  **`IZ Scan`** → **`All Applications`**.
...
1. Navigate to **`IZ Scan`** → **`All Applications`**.
2. Each row is one application version (IZ Eye) or one branch or pull request (IZ Scan):
