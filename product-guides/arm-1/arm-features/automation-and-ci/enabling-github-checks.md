...
Protected Branches are a feature in [GitHub](../../../arm/arm-features/automation-and-ci/webhooks/configure-a-webhook-in-github.md) that lets you block merging pull requests into specific branches unless status checks have passed.&#x20;
...
Let's say we have three branches: A, B, and C.

1. Branch B is configured in the CI job with status check API.
...
* Status check API is triggered as "Success" if the CI Job status is "Completed," "Partial.”
* Status check API is triggered as "Error" if the CI Job status is "Aborted."
