...
8. To download the **X509 Signing Certificate**, navigate to **Service > Certificates** under the ADFS Management Console.&#x20;
   1. Export the Token-signing certificates from ADFS and add them to the CodeScan SAML configuration. When using the certificate exporting wizard, select **Base-64 encoded X.509 (.CER)** for the encoding format.
   2.  Open the exported file in a text editor to get the certificate value.\
...
3. In the **Edit SAML Connection** dialog box on **CodeScan**, enter the following values:\
   \
   a. **Provider Entity Id:** Copy the **entityID** value from the ADFS Metadata XML file and paste it into **Provider Entity Id** inside CodeScan. \
