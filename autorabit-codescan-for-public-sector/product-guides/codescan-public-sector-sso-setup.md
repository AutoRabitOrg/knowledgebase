...
This guide is intended to be followed sequentially. Please note there is a combination of steps that must be performed by a CodeScan Org admin and an Azure AD / Entra ID admin.&#x20;

PART 1: Create SAML Group in AD (as needed) and make that SAML group available in Entra ID.&#x20;
...
* In Entra ID, create a single SAML IDP using the domain specified in CodeScan previously.
* Then, configure multiple email domains within this single SAML IDP (in Entra ID).
* NOTE: Later in this setup process, you will set up group sync via IDP in CodeScan.
