...
## CICD Integration - Using IZ Scan CLI
...
3. Run **`iz-scan-cli`** command with following options
   1. -serviceHost=xxx\
   ...
   6. -scmBranchName=xxx + _Optional. SCM branch for which code is being analysed. If ignored, the default value will be **`master`**_
...
If the system from which the projects are analyzed is configured with proxy, then set the following environment variable with proxy server details -
