...
{% hint style="warning" %}
Rules Playground functionality is part of Anypoint Studio plugin. Refer to [Installing Anypoint Studio](../../iz-suite/releases/anypoint-studio-plugin/) plugin for more information.
{% endhint %}
...
5. Click on **`Window`** -> **`Show View`** -> **`Other`** -> **`IZ Analyzer`** -> **`Custom Rule Editor`**
...
* Appropriate access in the server to publish the rule. Users with either **`Administer System`** or **`Quality Profile`** permission will be able to publish the rule.
{% endhint %}
...
* For Managing custom rules in server - [Custom Rules](custom-rules.md)
* For Activating rules - [Activate Rules](activate-rules.md)
* For Deactivating rules - [Deactivate Rules](deactivate-rules.md)
