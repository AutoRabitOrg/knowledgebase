...
You can stop the deployment if the SCA doesn't meet the criteria set globally. Go to **`My Account >`** [**`Validation Criteria- Static Code Analysis`**](https://knowledgebase.autorabit.com/~/changes/4385/product-guides/arm-1/getting-started-1/arm-administration/manage-users-account-settings) to set the global configuration for your SCA tool.
...
[**`Search and Substitute`**](../../registration/search-and-substitute.md)**`:`** If you have created the search and substitute rules to define custom find and substitute rules that ARM applies whenever you commit and deploy files from one Sandbox to another Sandbox, one Sandbox to Version Control, or vice versa, such rule can be found here.&#x20;
...
Click on **`Validate Deployment.`** You will be redirected to the [Track Deployment History](monitor-deployments.md) screen, showing the deployment progress.
